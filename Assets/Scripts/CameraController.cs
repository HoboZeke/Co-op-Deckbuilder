...
public class CameraController : MonoBehaviour
{
    ...
    {
        TurnOffHandCamera();
        StartCoroutine(MoveCamera(mainCamera.transform.position, lobbyPosition, mainCamera.transform.localEulerAngles, lobbyRotation, 2f));
    }

    public void MoveCameraToExploreMode()
    {
        TurnOffHandCamera();
        MoveCamera(positions[abstractCameraLoc], exploreModePos, rotations[abstractCameraLoc], exploreModeRot);
    }
    ...
    public void SetCameraPosition(Vector2 abstractPos)
    {
        ...
        {
            MoveCamera(positions[abstractCameraLoc], positions[abstractPos], rotations[abstractCameraLoc], rotations[abstractPos]);
    ...
    public void MoveCameraUp() { MoveCamera(new Vector2(0f, 1f)); }
    public void MoveCameraRight() { MoveCamera(new Vector2(1f, 0f)); }
    public void MoveCameraDown() { MoveCamera(new Vector2(0f, -1f)); }
    public void MoveCameraLeft() { MoveCamera(new Vector2(-1f, 0f)); }
    ...
    public void MoveCamera(Vector2 dir)
    {
        ...
        {
            MoveCamera(positions[abstractCameraLoc], positions[proposedDirection], rotations[abstractCameraLoc], rotations[proposedDirection]);
            ...
            CheckForValidCameraMoves();
        }
    }

    public void CheckForValidCameraMoves()
    {
        List<int> validMoves = new List<int>(); // 0=up, 1=down, 2=right, 3=left

        Vector2 up = abstractCameraLoc + new Vector2(0f, 1f);
        Vector2 down = abstractCameraLoc + new Vector2(0f, -1f);
        Vector2 right = new Vector2(abstractCameraLoc.x + 1f, 0f);
        Vector2 left = new Vector2(abstractCameraLoc.x - 1f, 0f);

        if (up.y == 2f) { up = new Vector2(0f, up.y); } //Balance out up for the location
        if (down.y == -1f) { down = new Vector2(0f, down.y); } //Balance out down for zoom out view

        if (positions.ContainsKey(up)) { validMoves.Add(0); }
        if (positions.ContainsKey(down)) { validMoves.Add(1); }
        if (positions.ContainsKey(right)) { validMoves.Add(2); }
        if (positions.ContainsKey(left)) { validMoves.Add(3); }

        UIController.main.ShowOnlyValidCameraControls(validMoves);
    }
    ...
    void MoveCamera(Vector3 from, Vector3 to, Vector3 fromRot, Vector3 toRot)
    {
        StartCoroutine(MoveCamera(from, to, fromRot, toRot, cameraMoveTime));
    }

    IEnumerator MoveCamera(Vector3 from, Vector3 to, Vector3 fromRot, Vector3 toRot, float moveTime)
    {
        isAnimating = true;
        ...
        {
            mainCamera.transform.position = Vector3.Lerp(from, to, timeElapsed / moveTime);
            mainCamera.transform.localEulerAngles = Vector3.Lerp(fromRot, toRot, timeElapsed / moveTime);
    ...
    IEnumerator MoveCameraToRecruitment()
    {
        ...
        UIController.main.ShowOnlyValidCameraControls(new List<int> { 1 });
    }
