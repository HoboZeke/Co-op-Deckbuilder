...
public class Card
{
    ...
    public void OnAttachTo(CardObject version, CardObject target)
    {
        ...
        if (version.ExtraCardEffects() != null)
        {
            foreach (CardEffect effect in version.ExtraCardEffects())
            {
                effect.AttachToEffect(version, target);
            }
        }
    ...
    public AbilityKeywords.Keyword[] CardKeywords()
    {
        ...
        if(gainCoinOnDefeat > 0) { keywords.Add(AbilityKeywords.Keyword.Bounty); }
    ...
    public string AbilityText()
    {
        ...
        foreach (CardEffect effect in cardEffects)
        {
            if (effect.DescriptionText() != "")
            {
                s += effect.DescriptionText();
                s += "\n";
            }
        }
    ...
    public string[] RawAbilityText()
    {
        List<string> strings = new List<string>();

        if(gainCoinOnDefeat > 0) { strings.Add("Bounty " +  gainCoinOnDefeat); }

        foreach(CardEffect effect in cardEffects)
        {
            strings.Add(effect.DescriptionText());
        }
