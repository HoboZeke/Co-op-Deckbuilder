...
public class CutsceneController : MonoBehaviour
{
    ...
    [SerializeField] Volume lobbyVolume;
    [SerializeField] float volumeBaseSetting, volumeEndSetting, fadeUpTime;
    ...
    public void FadeOutOfLobby()
    {
        StartCoroutine(FadeOutFromLobby());
    }

    IEnumerator FadeOutFromLobby()
    {
        float timeElapsed = 0f;
        ColorAdjustments colorAdjustments;
        if (!lobbyVolume.profile.TryGet<ColorAdjustments>(out colorAdjustments)) throw new System.NullReferenceException(nameof(colorAdjustments));

        while(timeElapsed < fadeUpTime)
        {
            colorAdjustments.postExposure.Override(Mathf.Lerp(volumeBaseSetting, volumeEndSetting, timeElapsed/fadeUpTime));
        ...
        colorAdjustments.postExposure.Override(volumeBaseSetting);
    }

    public void PlayIntroCutsceneIntoNewBattle(int profileID)
    {
        ...
        StartCoroutine(IntroCutscene(profileID));
    }

    IEnumerator IntroCutscene(int profileId)
    {
        ...
        GameController.main.StartBattle(profileId);
