...
public class EnemyDecks : MonoBehaviour
{
    ...
    public enum EnemyDeck { Beasts, SeaCreatures }
    ...
    public void StartBattle(BattleProfile profile)
    {
        ...
        {
            MultiplayerStartBattle(profile);
        }
    }

    void MultiplayerStartBattle(BattleProfile profile)
    {
        ...
        for (int i = 0; i < MutliplayerController.active.playerCount; i++)
        {
            ...
            int preRandomSpawns = 0;
            for(int j = 0; j < profile.reservedEnemyID.Length; j++)
            {
                array[j] = profile.reservedEnemyID[j];
                preRandomSpawns++;
            }

            for(int s = preRandomSpawns; s < array.Length; s++) {
        ...
        int locPreRandomSpawns = 0;
        for (int j = 0; j < profile.reservedEnemyID.Length; j++)
        {
            locationMonsters[j] = profile.reservedEnemyID[j];
            locPreRandomSpawns++;
        }

        for (int s = locPreRandomSpawns; s < locationMonsters.Length; s++)
        {
