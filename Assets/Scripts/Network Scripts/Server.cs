...
public class Server : NetworkBehaviour
{
    ...
    [ServerRpc(RequireOwnership = false)]
    public void StartTheBattleServerRpc(ulong playerId, int locationId)
    {
        StartYourBattleClientRpc(playerId, locationId, SendToAll());
    }
    ...
    [ServerRpc(RequireOwnership = false)]
    public void RequestBoardStateFromServerRpc(ulong playedId)
    {
        SyncBoardStatesClientRpc(0, GameController.main.CurrentBoardState(), SendToSpecific(playedId));
    }
    ...
    [ClientRpc]
    void StartYourBattleClientRpc(ulong fromPlayer, int locationID, ClientRpcParams sendToClients)
    {
        client.StartMultiplayerBattle(locationID);
    }
