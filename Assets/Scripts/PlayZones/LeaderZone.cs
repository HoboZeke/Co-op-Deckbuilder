...
public class LeaderZone : MonoBehaviour
{
    ...
    public void Setup(int leader)
    {
        ...
        leaderCard.SetZone(Zones.Type.Leader);
    }
    ...
    public void ReturnUsedLeaderCardToZone()
    {
        ...
        {
            if(MutliplayerController.active.playerCount == 1)
            {
                leaderCard.MarkCardAsUnavailableForTurns(2);
            }
            else if (MutliplayerController.active.playerCount == 2)
            {
                leaderCard.MarkCardAsUnavailableForTurns(1);
            }
        }

        StartCoroutine(AnimateMoveCardToZone(leaderCard, leaderSpent));
    }

    public void RefreshLeader()
    {
        Debug.Log("Refreshing leader for player " + zone.player.name);
        if (leaderCard.TurnsUnavailable() > 0)
        {
            leaderCard.TickTurnForUnavailableCounter(1);
        }
        else
        {
            leaderSpent = false;
        }
        StartCoroutine(RefreshLeader(leaderCard, leaderSpent));
    }
    ...
    IEnumerator AnimateMoveCardToZone(CardObject card, bool spent)
    {
        ...
        card.SetZone(Zones.Type.Leader);
    }
}
