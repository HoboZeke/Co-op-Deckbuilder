...
public class PlayArea : MonoBehaviour
{
    ...
    {
        //Set the offset manually, then modify it by increments of 10;
        float offset = 0.2f;
        int mod = cardCount / 6;
        offset = offset / (mod + 1);
    ...
    IEnumerator AnimatedMoveCardToPlayArea(CardObject card)
    {
        ...
        while (timeElapsed < moveDuration)
        {
            ...
            card.transform.localRotation = Quaternion.Lerp(startRot, endRot, timeElapsed / moveDuration);
        ...
        cardsMovingIntoPlay--;
        if (cardsMovingIntoPlay <= 0) { ShiftPositions(null); cardsMovingIntoPlay = 0; }

        card.SetZone(Zones.Type.Play);
