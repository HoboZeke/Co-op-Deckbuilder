...
public class Player : MonoBehaviour
{
    ...
    public int cardsDrawnAtStartOfTurn;
    ...
    //Sync Markers
    public int cardsToDrawBeforeReadyForSync;
    ...
    public void StartRound()
    {
        GainCoins(3);
        zones.deck.DrawCards(cardsDrawnAtStartOfTurn);
        zones.leader.leaderCard.ResetTurnsUnavailable();
        zones.leader.RefreshLeader();
        NewTurn();
    }
    ...
    public void NewTurn(bool sync)
    {
        remainingRecruits = recruits;
        UpdateRecruitsText();

        ResetShield();
        if (sync) { StartCoroutine(SyncWhenNewTurnSetupComplete()); }
    }

    IEnumerator SyncWhenNewTurnSetupComplete()
    {
        bool toSync = MutliplayerController.active.IsMultiplayerGame();

        while(toSync) 
        {
            if (zones.hand.DrawQueue() > 0 || cardsToDrawBeforeReadyForSync > 0) { yield return null; }
            else
            {
                GameController.main.PlayerReadyForNewTurnSync();
                toSync = false;
            }
        }
