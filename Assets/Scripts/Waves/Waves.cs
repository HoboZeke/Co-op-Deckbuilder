...
public class Waves : MonoBehaviour
{
    ...
    public bool generateOnStart, generateWaves, tryDeform;
    Vector3[] meshVerts;
    ...
    void AnimateWaves()
    {
        float xMod = animateDirection.x * Time.deltaTime * animateSpeed;
        float yMod = animateDirection.y * Time.deltaTime * animateSpeed;
        ...
        GenerateWavesWithDeformedVerts(false);
        //Deform();
        //mesh.RecalculateBounds();
        //mesh.RecalculateNormals();
    }
    ...
    void GenerateWavesWithDeformedVerts(bool random)
    {
        Vector3[] verts = mesh.vertices;
        if (random) seed = Time.time;
        maxY = 0f;
        ...
        for (int x = 0; x <= dimension; x++)
        {
            for (int z = 0; z <= dimension; z++)
            {
                if (!deformedVertPos.Contains(new Vector2((float)x, (float)z)))
                {
    ...
    List<Vector2> deformedVertPos = new List<Vector2>();
        ...
        if (!skipDeform)
        {
            ...
            for (int i = 0; i < meshVerts.Length; i++)
            {
                ...
                if (Physics.Raycast(ray, out hit, rayDist))
                {
                    ...
                    meshDeformedCount++;
                    deformedVertPos.Add(new Vector2(meshVerts[i].x, meshVerts[i].z));
